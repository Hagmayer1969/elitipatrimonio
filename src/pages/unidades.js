...
function deleteUnidade(id) {
  ...
  if (totalEqs > 0) {
    alert(`Não é possível remover "${u.nome}" pois possui ${totalEqs} equipamento(s) vinculado(s).`);
    return;
  }
...
function _resetModalUnidade() {
  document.querySelector("#modal-unidade .modal-head div").textContent = "Nova Unidade";
  const btn = document.querySelector("#modal-unidade .modal-foot .btn-primary");
