...
async function carregarDados() {
  ...
  try {
    const [eqRes, usrRes, unRes, movRes] = await Promise.all([
      ...
      sb.from("movimentacoes").select("*").order("data", { ascending: false }).limit(200),
    ]);
...
async function dbSalvarEquipamento(eq) {
  ...
  if (error) { console.error(error); showToast("⚠ Erro ao salvar equipamento."); return false; }
  return true;
}

async function dbDeletarEquipamento(id) {
  const { error } = await sb.from("equipamentos").delete().eq("id", id);
  if (error) { console.error(error); showToast("⚠ Erro ao deletar equipamento."); return false; }
...
async function dbAtualizarCampoEq(id, campos) {
  ...
  if (error) { console.error(error); showToast("⚠ Erro ao atualizar equipamento."); return false; }
...
async function dbSalvarUnidade(un) {
  ...
  if (error) { console.error(error); showToast("⚠ Erro ao salvar unidade."); return false; }
  return true;
}

async function dbDeletarUnidade(id) {
  const { error } = await sb.from("unidades").delete().eq("id", id);
  if (error) { console.error(error); showToast("⚠ Erro ao deletar unidade."); return false; }
...
async function dbSalvarUsuario(usr) {
  ...
  if (error) { console.error(error); showToast("⚠ Erro ao salvar aluno."); return false; }
  return true;
}
